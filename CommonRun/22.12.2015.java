...
##Returns the index of the first occurrence of needle in haystack, or -1 if needle is not part of haystack.(KMP)
    ...
    public int findSubStrIndex(String str,String subStr){
         ...
         return firstMatchedIndex<0?0:firstMatchedIndex; 
    ...
    public int[] getNextArr(String subStr){
        ...
        while(subffix_index<subStr_len){
...
## Symmetric Tree
    ...
    public boolean isSymmSub(TreeNode p, TreeNode q){
        ...
        if(p!=null && q!=null){
            return isSymmSub(p.val==q.val&&isSymmSub(p.right,q.left)&&isSymmSub(p.left,q.right);
        }
        return false;
    }
...
##HashMap iteration
    ...
    for(Entry<String,Sring> entry:map.entrySet()){
